...
public class LogIn extends Register{
    ...
    public static String[] readFiles1(String file) {// method for converting the data of the file into array and storing
        try {
            ...
            System.out.println(("--------------------------------------------"));
            System.out.println("\nEnter the Username:");
            String uname = sc.next();// here on using the scanner object sc we take the input from use
            for (int i = 0; i < arr.length; i++) {
                if (uname.equals(arr[i])) {// here it compares the input username with already existing username
                    flag = 1;
                    File fi=new File("C:\\Users\\USER\\Desktop\\tanmayee\\username1.txt");
                    write(uname,fi);
                    break;
                } else {
                    flag = 0;
