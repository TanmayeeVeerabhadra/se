 ...
 */
public class Reserve extends LogIn {
   ...
   // {System.out.println(uname1);}
    ...
    public static  String[] reserve(String file){// converting the data of the file into array and storing into the files 
        ...
        try{
            ...
            System.out.println("---------------------------------------------------");
            System.out.println("\nEnter the Bookname:");
            String bname=sc.next();//uname is the variable stores the bookname
            //System.out.print(uname);
            for(int i=0;i<arr.length;i++){
                //System.out.println(arr[i]);
                if(bname.equals(arr[i])){//making sure the book is in the library or not  
                    flag=1;
                    break;
                }
            ...
            for(int i=0;i<arr1.length;i++){//making sure if the book has been already reserved are not 
                //System.out.println(arr[i]);
                if(bname.equals(arr1[i])){//if found then the flag1 variable rises 
                    flag1=1;
                    break;
                }
            ...
            if(flag1==0){
                if(flag==1){
                    ...
                    //fil stores the roll number by whom the book has been reserved
                    if(fil.length()==10){
                        ...
                        if(res.equals("y")){//if res stores y ie if user want to reserve
                            ...
                            File fill=new File("C:\\Users\\User\\Desktop\\tanmayee\\reserve.txt");
                            //String s12=reader("username.txt");
                            write("The book " + bname + " has been reserved by "+ fil +" on "+ getDate()+" within the username "+reader("username1.txt") +"\n",fi);//creates a report by whom the book has been reserved 
                            writes(bname+"\n",fill);//stores the book name which has reserved 
                            System.out.println("The Book " + bname + " has been reserved by "+fil+ " on "+ getDate()+" within the UserName "+reader("username1.txt"));
